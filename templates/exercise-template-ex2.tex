...
Show from (\ref{density_operator}) and (\ref{canonical_density_matrix}) that the $p_i$ are indeed equal to $\frac{1}{Z}e^{ -\beta E_i}$ for mixed states, given that there exists a common eigenbasis $\left\{\ket{\Psi_i}\right\}$ between the total Hamiltonian and (\ref{density_operator}) and explain the origin of this restriction.
